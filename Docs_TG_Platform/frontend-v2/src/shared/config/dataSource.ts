/**
 * Data source modes (see greenfield plan):
...
/** MSW intercepts fetch in the browser; default on in development. */
export const USE_MSW =
  process.env.NEXT_PUBLIC_USE_MSW === "1" ||
  (process.env.NEXT_PUBLIC_USE_MSW !== "0" && process.env.NODE_ENV === "development");

/** Base URL for REST API (no trailing slash). Empty = same-origin relative paths (MSW). */
...
/** Real backend mode: HTTP client talks to API_BASE_URL without MSW. */
