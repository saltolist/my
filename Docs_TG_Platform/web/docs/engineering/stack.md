...
**UX reference:** [`web-legacy`](../../../web-legacy/) — визуал и поведение (см. [pages.md](../ux/pages.md)).

**Active codebase:** [`web/src/`](../../src/) — Next.js app (`npm run dev` в `Docs_TG_Platform/web/`).
...
### Стилевая стратегия (product UI)

| Слой | Роль |
|------|------|
| **`tokens.css`** | источник правды: цвета, glass, `--page-header-*`, `--nav-w` |
| **`shell-*.css` + widget CSS** | визуал legacy: `.page-header`, `.nav-item`, `.btn`, `.ctx-dropdown` |
| **Tailwind `@theme`** | мост токенов → utility (`bg-background`, layout) |
| **shadcn** | headless/поведение; не дефолтный визуал в product shell |

Правила для PR:

- Shell и виджеты — **semantic CSS-классы**, не shadcn variants в JSX
- Новые экраны — порт CSS из `web-legacy/src/app/styles/*.css`
- Utility Tailwind — **layout** (`flex`, `min-w-0`), не «дизайн» (`rounded-xl`, `bg-zinc-*`)
- Product dropdowns в шапке — `ContextMenu` + `.ctx-dropdown--page-header-control`, не shadcn `DropdownMenu`
...
| URL navigation | **Zustand** | `app/model/store/navigation-store.ts` |
