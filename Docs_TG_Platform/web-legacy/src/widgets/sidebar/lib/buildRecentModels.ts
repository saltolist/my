...
export function buildRecentChatsModel(
  ...
  let gSeq = 0;
  for (const c of domain.globalChats) {
    globalRows.push({
      kind: "global",
      key: `g:${c.id}`,
      id: c.id,
  ...
  const inPostSpace =
    sidebarPostId != null && domain.posts.some((p) => p.id === sidebarPostId);

  if (!inPostSpace) {
    const allLocal: RecentRow[] = [];
    let seq = 0;
    for (const p of domain.posts) {
  ...
  const post = domain.posts.find((p) => p.id === sidebarPostId)!;
...
export function buildRecentNotesModel(
  ...
  let gSeq = 0;
  for (const n of domain.globalNotes) {
    globalRows.push({
      kind: "global",
      key: `ng:${n.id}`,
      id: n.id,
