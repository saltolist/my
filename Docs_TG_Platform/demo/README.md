...
- `posts` (включая `media: PostMedia[]` и `chats: LocalChat[]` — массив локальных чатов поста), `globalChats`, `globalNotes`, `aiProfileConfig`, `telegramProfileConfig` — мок-сущности;
- `screen`, `currentPostId`, `currentPostChatId`, `currentGChatId`, `currentNote`, `postMode` (`chat` / `chats` / `notes`), `postViewStack` (стек переходов внутри пространства поста), `noteMode`, и т. п. — UI-состояние;
...
- **пространство поста**: три режима в одной шапке (`📋 Заметки` / `💬 Чаты` + по умолчанию `chat`); первое сообщение — карточка поста с медиа сверху и inline-редактированием (textarea авто-ресайзится от исходного размера, медиа остаётся сверху, кнопка `🖼 Добавить медиа`); множественные локальные чаты (каждый — отдельный контекст для ИИ; новый чат создаётся первым отправленным сообщением или через `+ Новый чат` / пункт меню `•••`); стек переходов внутри пространства (кнопка `← Назад` сначала разматывает историю режимов/чатов, потом уходит на ленту);
...
- **единая шапка** (`PageHeader`) в стиле liquid glass: заголовок или хлебные крошки слева, опциональный поиск по центру (на `feed` — выровнен по колонке постов), `← Назад` / `↑ К посту` / переключатели режимов / `•••` справа;
