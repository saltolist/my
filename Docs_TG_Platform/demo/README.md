...
- **лента**: три секции, TG-стиль медиа, пилюли реакций + `👁`/`↗`/`💬` у опубликованных, поиск, drag-and-drop черновиков, composer для нового черновика;
...
- **единая шапка** (`PageHeader`) в стиле liquid glass: крошки слева, поиск по центру, `↑ К посту` / режимы / `← Назад` / `•••` справа;
...
- **чаты**: вкладки `Глобальные/Локальные` (по карточке на каждый локальный чат), поиск в шапке, переходы в конкретный глобальный чат или в локальный чат внутри пространства поста;
