...
import type { MentionPos } from "@/components/composer/ComposerMentionDropdown";
import {
  createChipElement,
  nextAttachId,
  serializeChip,
} from "@/components/composer/composerChipUtils";
import {
  attachmentPostTitle,
  postFreshness,
  postTitle,
...
export function useComposerEditor({ scope, placeholder, onSubmit }: Props) {
  ...
  }, [state.posts, currentPostId, scope, attachedPostIds]);
  ...
  const removeChipById = useCallback(
    ...
    [pruneOrphanComposerSpaces, refreshIsEmpty, syncEmptyEditorDom],
  );

  const detectMentionFromSelection = useCallback((): MentionState => {
    const editor = editorRef.current;
  ...
  const collectAttachmentsFromDom = useCallback((): ComposerAttachment[] => {
    const editor = editorRef.current;
    if (!editor) return [];
  ...
  const serializeEditor = useCallback((): string => {
    const editor = editorRef.current;
    if (!editor) return "";
  ...
  const addAttachmentInline = useCallback(
    ...
    [placeCaretAfter, refreshIsEmpty, removeChipById],
  );
  ...
  const pickMention = useCallback(
    ...
    [mention, refreshIsEmpty, removeChipById],
  );
  ...
  const onKeyDown = useCallback(
    ...
    [mention, mentionMatches, mentionIndex, narrowComposer, pickMention, submit],
  );
